  ...
  const fetchProducts = async () => {
    try {
      ...
      try {
        ...
        // Use demo products on parse error
        setProducts(DEMO_PRODUCTS);
        setAdminProducts(DEMO_PRODUCTS);
        setLoading(false);
        return;
      }
      if (data.error) {
        ...
        setProducts(DEMO_PRODUCTS);
        setAdminProducts(DEMO_PRODUCTS);
        showToast('Работаем в демо-режиме');
      } else {
        setProducts(Array.isArray(data) ? data : DEMO_PRODUCTS);
        setAdminProducts(Array.isArray(data) ? data : DEMO_PRODUCTS);
      }
    } catch (e: any) {
      setProducts(DEMO_PRODUCTS);
      setAdminProducts(DEMO_PRODUCTS);
  ...
  if (isAdmin) {
    return (
      <div style={styles.adminContainer}>
            ...
            {adminProducts.map(product => (
              <div key={product.id} style={styles.adminItem}>
                ...
                <div style={styles.adminActions}>
                  <button 
                    style={{...styles.adminBtn, ...(product.in_stock ? styles.btnOut : styles.btnIn)}}
        ...
        {toast && <div style={{...styles.toast, borderColor: toast.type === 'success' ? '#44ff88' : '#2a2a2a'}}>{toast.message}</div>}
      </div>
    );
  }
  ...
  return (
    <div>
      <header style={styles.header}>
        ...
        <nav>
          <a style={styles.navLink} onClick={() => setFilter('all')}>Каталог</a>
        </nav>
      </header>
      ...
      <div style={styles.catalog}>
        {filteredProducts.map((product, i) => (
          <div 
            key={product.id} 
            style={{...styles.productCard, opacity: product.in_stock ? 1 : 0.6, animationDelay: `${i * 0.1}s`}}
          ...
          >
            <div style={styles.productImageWrap}>
              <img 
                src={product.image} 
                alt={product.title} 
                style={{...styles.productImage, filter: product.in_stock ? 'none' : 'grayscale(100%)'}} 
              />
            </div>
        ...
        <div style={styles.modalOverlay} onClick={() => setSelectedProduct(null)}>
          <div style={styles.modal} onClick={e => e.stopPropagation()}>
            ...
            <div style={styles.modalContent}>
              ...
              <button 
                style={{...styles.btn, ...(selectedProduct.in_stock ? {} : styles.btnDisabled)}}
...
const styles: Record<string, React.CSSProperties> = {
  loader: {
    position: 'fixed', inset: 0, background: '#0a0a0a', display: 'flex', alignItems: 'center', justifyContent: 'center',
  },
  loaderText: {
    fontFamily: 'var(--font-bebas)', fontSize: '48px', letterSpacing: '8px', animation: 'pulse 1s infinite',
  },
  header: {
    position: 'fixed', top: 0, left: 0, right: 0, zIndex: 100, padding: '20px 40px', display: 'flex', justifyContent: 'space-between', alignItems: 'center',
    background: 'rgba(10,10,10,0.8)', backdropFilter: 'blur(20px)', borderBottom: '1px solid #2a2a2a',
  },
  logo: {
    fontFamily: 'var(--font-bebas)', fontSize: '32px', letterSpacing: '4px', cursor: 'pointer',
  },
  navLink: {
    color: '#888', fontSize: '14px', fontWeight: 500, marginLeft: '30px', cursor: 'pointer',
  },
  hero: {
    padding: '180px 40px 80px', textAlign: 'center',
  },
  heroTitle: {
    fontFamily: 'var(--font-bebas)', fontSize: 'clamp(48px, 10vw, 120px)', letterSpacing: '12px', lineHeight: 1,
  },
  heroSubtitle: {
    color: '#888', fontSize: '16px', marginTop: '20px',
  },
  filters: {
    padding: '0 40px', marginBottom: '40px', display: 'flex', gap: '12px', justifyContent: 'center', flexWrap: 'wrap',
  },
  filterBtn: {
    background: '#141414', border: '1px solid #2a2a2a', color: '#888', padding: '12px 24px', fontSize: '14px', fontWeight: 500, cursor: 'pointer', transition: 'all 0.3s',
  },
  filterBtnActive: {
    background: '#fff', color: '#0a0a0a', borderColor: '#fff',
  },
  catalog: {
    padding: '0 40px 80px', display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '30px', maxWidth: '1400px', margin: '0 auto',
  },
  productCard: {
    background: '#141414', border: '1px solid #2a2a2a', cursor: 'pointer', transition: 'all 0.4s', animation: 'fadeIn 0.6s forwards',
  },
  productImageWrap: {
    position: 'relative', aspectRatio: '1', overflow: 'hidden', background: '#0a0a0a',
  },
  productImage: {
  ...
  productStatus: {
    display: 'inline-block', fontSize: '11px', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '1px', padding: '4px 10px', marginTop: '12px',
  },
  modalOverlay: {
    position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.85)', backdropFilter: 'blur(10px)', zIndex: 200, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px',
  },
  modal: {
    background: '#141414', border: '1px solid #2a2a2a', maxWidth: '500px', width: '100%', maxHeight: '90vh', overflowY: 'auto', position: 'relative',
  },
  modalClose: {
    position: 'absolute', top: '20px', right: '20px', background: 'none', border: 'none', color: '#888', fontSize: '24px', cursor: 'pointer', zIndex: 10,
  },
  modalImage: {
  ...
  modalTitle: {
    fontFamily: 'var(--font-bebas)', fontSize: '32px', letterSpacing: '2px', marginBottom: '10px',
  },
  modalPrice: {
  ...
  btn: {
    background: '#fff', color: '#0a0a0a', border: 'none', padding: '16px 32px', fontSize: '14px', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '2px', cursor: 'pointer', width: '100%',
  },
  btnDisabled: {
    background: '#2a2a2a', color: '#888', cursor: 'not-allowed',
  },
  formGroup: {
    marginBottom: '20px',
  },
  input: {
    width: '100%', background: '#0a0a0a', border: '1px solid #2a2a2a', color: '#fff', padding: '14px 16px', fontSize: '14px',
  },
  textarea: {
    width: '100%', background: '#0a0a0a', border: '1px solid #2a2a2a', color: '#fff', padding: '14px 16px', fontSize: '14px', minHeight: '100px', resize: 'vertical',
  },
  loginForm: {
    background: '#141414', border: '1px solid #2a2a2a', padding: '40px', maxWidth: '400px', width: '100%',
  },
  toast: {
    position: 'fixed', bottom: '30px', left: '50%', transform: 'translateX(-50%)', background: '#141414', border: '1px solid #2a2a2a', padding: '16px 32px', zIndex: 300,
  },
  adminContainer: {
  ...
  adminTitle: {
    fontFamily: 'var(--font-bebas)', fontSize: '48px', letterSpacing: '4px', marginBottom: '40px',
  },
  addForm: {
    background: '#141414', border: '1px solid #2a2a2a', padding: '30px', marginBottom: '40px',
  },
  addFormGrid: {
    display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '16px', marginTop: '20px',
  },
  adminList: {
    display: 'grid', gap: '20px',
  },
  adminItem: {
    background: '#141414', border: '1px solid #2a2a2a', padding: '20px', display: 'grid', gridTemplateColumns: '80px 1fr auto', gap: '20px', alignItems: 'center',
  },
  adminImg: {
  ...
  adminBtn: {
    background: '#0a0a0a', border: '1px solid #2a2a2a', color: '#888', padding: '10px 16px', fontSize: '12px', fontWeight: 500, cursor: 'pointer',
  },
  btnIn: {
    background: 'rgba(68,255,136,0.15)', borderColor: '#44ff88', color: '#44ff88',
  },
  btnOut: {
    background: 'rgba(255,68,68,0.15)', borderColor: '#ff4444', color: '#ff4444',
  },
  btnDelete: {
